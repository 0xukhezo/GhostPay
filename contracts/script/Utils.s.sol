...
contract Utils is Script {
    ...
    function getAddressesFromConfigJson(string memory key) public view returns (address[] memory) {
        string memory json = getConfigJson();
        bytes memory data = json.parseRaw(key);
        return abi.decode(data, (address[]));
    }

    function getUintsFromConfigJson(string memory key) public view returns (uint256[] memory) {
        string memory json = getConfigJson();
        bytes memory data = json.parseRaw(key);
    ...
    function getUint8FromConfigJson(string memory key) public view returns (uint8) {
        string memory json = getConfigJson();
        bytes memory data = json.parseRaw(key);
        return abi.decode(data, (uint8));
    }

    function getBytes32MatrixFromConfigJson(string memory key) public view returns (bytes32[][] memory) {
        string memory json = getConfigJson();
        bytes memory data = json.parseRaw(key);
    ...
    function getJsonConfigPathByChain(uint256 chainId) public view returns (string memory) {
        string memory inputDir = string.concat(vm.projectRoot(), "/addresses/");
        string memory chainDir = string.concat(vm.toString(chainId), "/");
        string memory file = string.concat("config.json");
        return string.concat(inputDir, chainDir, file);
    }
